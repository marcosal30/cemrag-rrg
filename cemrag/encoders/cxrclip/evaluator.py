...
class Evaluator:
    def __init__(self, config: Dict, ckpt_paths):
        ...
        # load dataset
        data_config = {"test": {self.config["defaults"][1]["data_test"]: {}}}
        
      

        data_config["test"][self.config["defaults"][1]["data_test"]]["normalize"] = "imagenet"
        
        self.ckpt_config["tokenizer"]['pretrained_model_name_or_path']='./tokenizer'
        self.datamodule = DataModule(
            data_config=data_config,
            dataloader_config=self.config["defaults"][2]["dataloader"],
    ...
    def evaluate_clip(self, checkpoint, test_dataset_name,model):
        self.model=model
        self.model.eval()
