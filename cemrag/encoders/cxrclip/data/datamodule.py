    ...
    def __init__(
        ...
        for split in data_config:
            ...
            for name in dataset_split_config:
                ...
                dataset = load_dataset(data_type="imagetext_eval",
                    split=split, tokenizer=self.tokenizer, transform_config=transform_config, loss_config=self.loss_config,data_path=self.data_path, **dataset_config
                )
                self.datasets[split].append(dataset)

                log.info(f"Dataset loaded: {dataset_split_config[name]} for {split}")
        ...
        if self.test_loader is None:
            self.test_loader = {
                test_dataset.name: DataLoader(
                    test_dataset, collate_fn=getattr(test_dataset, "collate_fn", None)
                )
