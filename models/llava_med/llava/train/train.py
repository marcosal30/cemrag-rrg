...
from model import *
from mm_utils import tokenizer_image_token, open_image_with_retry
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
                #image = processor.preprocess(image) #this is for CXR-CLIP
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.bits in [4, 8]:
            model.get_model().mm_projector.to(dtype=compute_dtype, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
